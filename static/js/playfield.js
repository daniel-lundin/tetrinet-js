...
PlayField.prototype.shape_at = function (x, y) {
  for (var i = 0; i < this.shape.length; ++i) {
    for (var j = 0; j < this.shape[i].length; ++j) {
      if (this.shape[i][j]) {
        if (i + this.shape_pos_x == x && j + this.shape_pos_y == y) {
  ...
  for (var x = 0; x < shape.length; ++x) {
    for (var y = 0; y < shape[x].length; ++y) {
      if (shape[x][y]) {
        ...
        var realy = y + yoffset;
        // Check borders
        if (realx < 0 || realx >= this.width) return false;
        if (realy < 0 || realy >= this.height) return false;
        var cellidx = realy * this.width + realx;
        if (this.cells[cellidx]) return false;
      }
    }
  }
  return true;
